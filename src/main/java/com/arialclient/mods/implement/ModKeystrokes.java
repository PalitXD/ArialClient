...
public class ModKeystrokes extends ModDraggable {
	...
	public static enum KeystrokesMode {
		...
		WASD_JUMP(Key.W, Key.A, Key.S, Key.D, Key.LMB, Key.RMB, new Key("§m----", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 41, 58, 15, false)),
		WASD_JUMP_MOUSE(Key.W, Key.A, Key.S, Key.LMB, Key.RMB, Key.D, new Key("§m----", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 61, 58, 15, false));
	...
	private static class Key {
		...
		private static final Key LMB = new Key("LMB", Minecraft.getMinecraft().gameSettings.keyBindAttack, 1, 41, 28, 21, true);
		private static final Key RMB = new Key("RMB", Minecraft.getMinecraft().gameSettings.keyBindUseItem, 31, 41, 28, 21, true);
		...
		for(Key key : mode.getKeys()) {
			...
			if(key.cps) {
				...
				if(key.getName().matches(key.LMB.getName())) {
					font.drawString("   CPS " + getCPS(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 25 - textWidth / 25 , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 8, -1);
				}

				if(key.getName().matches(key.RMB.getName())) {
					font.drawString("   CPS " + getCPS2(), pos.getAbsoluteX() + key.getX() + key.getWidth() / 25 - textWidth / 25 , pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 + 8, -1);
				}
