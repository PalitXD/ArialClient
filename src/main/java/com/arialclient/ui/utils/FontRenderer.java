...
public class FontRenderer {
    ...
    private Font getFontByName(String name) throws IOException, FontFormatException {
        if (name.equalsIgnoreCase("typographica"))
            return getFontFromInput("/assets/minecraft/arialclient/fonts/TypoGraphica.ttf");
        else
            return getFontFromInput("/assets/minecraft/arialclient/fonts/TypoGraphica.ttf");
    }

    private Font getFontFromInput(String path) throws IOException, FontFormatException {
        return Font.createFont(Font.TRUETYPE_FONT, Objects.requireNonNull(FontRenderer.class.getResourceAsStream(path)));
    }
